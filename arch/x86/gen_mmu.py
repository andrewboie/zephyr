...
mapped with the Present and Write bits set.
...
def main():
    ...
    is_vm = isdef("CONFIG_VIRTUAL_MEMORY")
    is_xip = isdef("CONFIG_XIP")

    ptables_virt = syms["z_x86_pagetables_start"]
    if is_vm:
        # Covert to physical address. virt_base also used to setup
        # dual mapping for early boot transition
    ...
    if is_perm_regions:
        # Don't allow execution by default for data pages
    ...
    if is_xip:
        if is_vm:
            error("XIP and VIRTUAL_MEMORY are currently incompatible")
        # Just identity map everything
        pt.map(syms["CONFIG_FLASH_BASE_ADDRESS"], None,
               syms["CONFIG_FLASH_SIZE"] * 1024, map_flags)

    # Adjust mapped region permissions
    if is_perm_regions:
        ...
        # has no access to any kernel memory, but will require different
        # linking
        pt.set_region_perms("_image_text", FLAG_P | FLAG_US)
        pt.set_region_perms("_image_rodata", FLAG_P | FLAG_US | FLAG_XD)

        if "CONFIG_X86_64" in syms:
            pt.set_region_perms("_locore", FLAG_P | FLAG_US)
            pt.set_region_perms("_lorodata", FLAG_P | FLAG_XD | FLAG_US)
