...
uintptr_t arch_page_info_get(void *addr, uintptr_t *phys, bool clear_accessed)
{
	...
	ret = page_map_set(z_x86_kernel_ptables, addr, 0, &all_pte, mask,
			   options);
	__ASSERT(ret == 0, "page_map_set fail in %s for %p", __func__,
		 z_x86_kernel_ptables);
	(void)ret;
	...
	if ((all_pte & MMU_P) != 0) {
		...
		SYS_SLIST_FOR_EACH_NODE(&x86_domain_list, node) {
			...
			ret = page_map_set(domain->ptables, addr, 0, &cur_pte,
					   mask, options | OPTION_USER);
			__ASSERT(ret == 0,
				"page_map_set fail in %s for %p", __func__,
				domain->ptables);
			(void)ret;
