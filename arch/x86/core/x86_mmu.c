 ...
 *
 * FIXME: re-tune defaults, remove the 128 hacked in below
 */
 ...
 *                 If KPTI is not enabled this has no effect.
...
/*
 * Map or update page table entries for a specific set of page tables.
 *
 * Must call this with x86_mmu_lock held
 */
 ...
 * paging level.
 *
 * src is some paging structure from within the source page tables to copy.
 *
...
static void set_stack_perms(struct k_thread *thread)
{
	LOG_DBG("update stack permissions for thread %p", thread);
	apply_region(thread->mem_domain_info.mem_domain->arch.ptables,
...
int arch_mem_domain_init(struct k_mem_domain *domain)
{
	...
	SYS_SLIST_FOR_EACH_NODE(&x86_domain_list, node) {
		...
		__ASSERT(list_domain != &domain->arch,
			 "arch_mem_domain_init(%p) called multiple times",
			 domain);
	}
#endif /* __ASSERT_ON */
...
void arch_mem_domain_thread_add(struct k_thread *thread)
{
	 ...
	 * and have to update stack permissions.
	 *
	 ...
	 */
	if (old_ptables != NULL && (thread->base.user_options & K_USER) != 0) {
		/* Allow thread access its stack buffer in the incoming
		 * memory domain. We do this first as the thread might be
		 * running on another CPU.
		 */
		set_stack_perms(thread);

		/* Clear access in its previous set of page tables */
