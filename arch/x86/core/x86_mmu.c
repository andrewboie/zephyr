...
#define PT_MEM	(Z_X86_NUM_TABLE_PAGES * CONFIG_MMU_PAGE_SIZE) + 0x20
#else
#define PT_MEM	(Z_X86_NUM_TABLE_PAGES * CONFIG_MMU_PAGE_SIZE)
#endif
Z_GENERIC_SECTION(.dummy_pagetables) char z_x86_dummy_pagetables[PT_MEM];
		...
		if (entry & MMU_P) {
			if (is_leaf(level, entry)) {
				if (phys == virt) {
					/* Identity mappings */
					COLOR(YELLOW);
				} else if (IS_ENABLED(CONFIG_VIRTUAL_MEMORY) &&
					   phys + Z_MEM_VM_OFFSET == virt) {
...
int arch_mem_map(void *virt, uintptr_t phys, size_t size, uint32_t flags)
{
	 ...
	 */
	__ASSERT((uintptr_t)virt + (size - 1) <= 0x0000FFFFFFFFFFFFULL ||
		 (uintptr_t)virt >= 0xFFFF000000000000ULL,
		 "bad virtual address mapping %p (size %zu)", virt, size);
#endif /* CONFIG_X86_64 */
	...
	for (size_t offset = 0; offset < size; offset += CONFIG_MMU_PAGE_SIZE) {
		...
		ret = page_map_set(ptables, (uint8_t *)virt + offset, entry_val,
				   page_pool_get, NULL);
		if (ret != 0) {
...
#else
#define RAM_BASE	PHYS_RAM_ADDR
#endif
#define RAM_END		RAM_BASE + PHYS_RAM_SIZE
		       ...
		       pentry_t flags, bool flush)
{
	 ...
	 */
	__ASSERT((uintptr_t)ptr >= (uintptr_t)RAM_BASE,
		 "%p below system RAM", ptr);
	__ASSERT((uintptr_t)ptr < (uintptr_t)RAM_END,
...
static void page_reset(struct k_thread *thread, void *virt)
{
	 ...
	 */
	if ((kern_pte & MMU_US) == 0 && virt != &z_shared_kernel_page_start) {
		kern_pte = 0;
	}
