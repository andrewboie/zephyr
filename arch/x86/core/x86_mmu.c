...
/* Number of PDPTs needed to cover the adress space Depends on the
 * specific bounds of system RAM, but roughly 1 PDPT per 512GB of VM
 */
			 ...
			 pentry_t *old_val_ptr, pentry_t mask, uint32_t options)
{
	...
	for (int level = 0; level < NUM_LEVELS; level++) {
		...
		__ASSERT(table != NULL, "missing page table level %d when trying to map %p",
			 level + 1, virt);
	}
	if (flush) {
		tlb_flush_page(virt);
