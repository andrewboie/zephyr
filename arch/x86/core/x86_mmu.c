...
static void dump_entry(int level, void *virt, pentry_t entry)
{
	...
	#define DUMP_BIT(bit) if ((entry & MMU_##bit) != 0U) \
...
int arch_mem_map(void *virt, uintptr_t phys, size_t size, uint32_t flags)
{
	...
	LOG_DBG("arch_mem_map: %p -> %p (%zu) flags 0x%x",
		(void *)phys, virt, size, flags);

#ifdef CONFIG_X86_64
	/* There's a gap in the "64-bit" address space, as 4-level paging
	 * requires bits 48 to 63 to be copies of bit 47.
	 */
	__ASSERT((uintptr_t)virt + (size - 1) <= ((1UL << 47) - 1) ||
		 (uintptr_t)virt >= 0xFFFF80000000000ULL,
	...
	for (size_t offset = 0; offset < size; offset += CONFIG_MMU_PAGE_SIZE) {
		...
		if (flags & K_MEM_MAP_FLUSH) {
			tlb_flush_page(dest_virt);
		}
