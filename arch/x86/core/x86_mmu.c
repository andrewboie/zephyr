...
/* The "hole" in the 64-bit address space for 4-level IA32e paging starts
 * after this, up to its 1's complement
 */
#define MAX_IA32E_ADDR ((1ULL << 47) - 1)
...
int arch_mem_map(void *virt, uintptr_t phys, size_t size, uint32_t flags)
{
	 ...
	 */
	__ASSERT((uintptr_t)virt + (size - 1) <= MAX_IA32E_ADDR ||
		 (uintptr_t)virt >= ~MAX_IA32E_ADDR,
