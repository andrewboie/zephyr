...
static void free_page_frame_list_put(struct z_page_frame *pf)
{
	...
	PF_ASSERT(pf, pf->refcount == 0, "still has references");
...
{;
	...
	__ASSERT(size % CONFIG_MMU_PAGE_SIZE == 0,
		 "unaligned size %zu passed to k_mem_map()", size);
	__ASSERT(size != 0, "zero sized memory mapping");
	__ASSERT(page_frames_initialized, "k_mem_map() called too early");
	__ASSERT((flags & K_MEM_CACHE_MASK) == 0,
		 "k_mem_map does not support explicit cache settings");
