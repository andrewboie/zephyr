...
static void tmslab_api(void *p1, void *p2, void *p3)
{
	...
	while (j--) {
		...
		for (int i = 0; i < BLK_NUM; i++) {
			/* change to K_FOREVER to get it to stop crashing */
			ret = k_mem_slab_alloc(slab, &block[i], TIMEOUT);
			zassert_false(ret, "memory is not allocated");
		}
		for (int i = 0; i < BLK_NUM; i++) {
			/* Comment out this if() statement and just free
			 * unconditionally to get it to stop crashing
			 */
			if (block[i]) {
	...
	/* Uncomment to get this to stop crashing */
	//k_sleep(K_MSEC(100));
}
